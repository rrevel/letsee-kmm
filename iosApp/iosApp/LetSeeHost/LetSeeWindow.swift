...
final class LetSeeWindow: UIWindow {
    ...
    private enum Layout {
        ...
        static let innerButtonSize: CGFloat = 52
        ...
        static let pathLabelFont = UIFont(name: "Poppins-Medium", size: 11)
            ?? UIFont.systemFont(ofSize: 11, weight: .medium)
        static let pillFont = UIFont(name: "Poppins-Medium", size: 12)
            ?? UIFont.systemFont(ofSize: 12, weight: .medium)
        static let badgeFont = UIFont(name: "Poppins-Medium", size: 10)
    ...
    private let buttonShellView: UIView = {
        let v = UIView()
    ...
    private let button: UIButton = {
        ...
        // Prefer Poppins for the See badge; fallback keeps behavior safe when font isn't bundled.
        btn.titleLabel?.font = UIFont(name: "Poppins-Medium", size: 18)
    ...
    override init(windowScene: UIWindowScene) {
        ...
        updateVisualState(isMockEnabled: false)
    ...
    private func buildLayout(in parent: UIView) {
        ...
        // --- FAB shell + inner button ---
        containerView.addSubview(buttonShellView)
        buttonShellView.addSubview(button)
        NSLayoutConstraint.activate([
            buttonShellView.widthAnchor.constraint(equalToConstant: Layout.buttonSize),
            buttonShellView.heightAnchor.constraint(equalToConstant: Layout.buttonSize),
            buttonShellView.leadingAnchor.constraint(equalTo: containerView.leadingAnchor),
            buttonShellView.trailingAnchor.constraint(equalTo: containerView.trailingAnchor),
            buttonShellView.topAnchor.constraint(equalTo: containerView.topAnchor),
            buttonShellView.bottomAnchor.constraint(equalTo: containerView.bottomAnchor),

            button.widthAnchor.constraint(equalToConstant: Layout.innerButtonSize),
            button.heightAnchor.constraint(equalToConstant: Layout.innerButtonSize),
            button.centerXAnchor.constraint(equalTo: buttonShellView.centerXAnchor),
            button.centerYAnchor.constraint(equalTo: buttonShellView.centerYAnchor),
        ])
        ...
        NSLayoutConstraint.activate([
            ...
            badgeLabel.centerXAnchor.constraint(equalTo: buttonShellView.trailingAnchor, constant: -4),
            badgeLabel.centerYAnchor.constraint(equalTo: buttonShellView.topAnchor, constant: 4),
        ])
    ...
    private func refreshState() {
        ...
        let isMockEnabled = LetSeeIOSHelpersKt.isMockEnabled(letSee: letSee)

        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.updateVisualState(isMockEnabled: isMockEnabled)
            self.updateBadge(count: pendingCount)
            self.updateQuickAccess(data: quickAccess, letSee: letSee)
        }
    }

    private func updateVisualState(isMockEnabled: Bool) {
        let activeColor = UIColor(red: 0.59, green: 0.70, blue: 0.29, alpha: 1.0)
        let inactiveColor = UIColor(red: 0.82, green: 0.84, blue: 0.87, alpha: 1.0)
        let shellColor = isMockEnabled ? activeColor : inactiveColor

        let darkMode = traitCollection.userInterfaceStyle == .dark
        let innerBackground = darkMode ? UIColor.white : UIColor.black
        let innerTextColor = darkMode ? UIColor.black : UIColor.white

        buttonShellView.backgroundColor = shellColor
        quickAccessCard.backgroundColor = shellColor
        button.backgroundColor = innerBackground
        button.setTitleColor(innerTextColor, for: .normal)
    }
