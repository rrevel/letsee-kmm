...
class RequestTest {
    ...
    @Test
    fun `displayName when shouldCutBaseURLFromURLsTitle is true replaces baseURL with placeholder`() {
        val request = DefaultRequest(
            emptyMap(), "GET", "https://api.example.com/users/123", path = "/users/123"
        )
        val config = Configuration(false, true, "https://api.example.com")
        assertEquals("[BaseURL]/users/123", request.displayName(config))
    }

    @Test
    fun `displayName preserves original path casing after baseURL replacement`() {
        val request = DefaultRequest(
            emptyMap(), "GET", "https://api.example.com/Users/MyPath", path = "/Users/MyPath"
        )
        val config = Configuration(false, true, "https://api.example.com")
        assertEquals("[BaseURL]/Users/MyPath", request.displayName(config))
    }

    @Test
    fun `displayName replaces only first occurrence of baseURL`() {
        val request = DefaultRequest(
            emptyMap(), "GET", "https://api.example.com/redirect/https://api.example.com/target",
            path = "/redirect/https://api.example.com/target"
        )
        val config = Configuration(false, true, "https://api.example.com")
        assertEquals("[BaseURL]/redirect/https://api.example.com/target", request.displayName(config))
    }
